...
public class Arquivo<T extends aed3.InterfaceRegistro> {
    ...
    public Arquivo(String nomeEntidade, Constructor<T> construtor) throws Exception {
        // Comportamento original: tudo dentro de ./dados
        this("./dados", nomeEntidade, construtor);
    }

    // Adaptacao feita por nos: permite escolher a pasta base.
    // Precisamos disso para os testes automatizados, que usam pasta temporaria.
    public Arquivo(String pastaBase, String nomeEntidade, Constructor<T> construtor) throws Exception {
        File f = new File(pastaBase);
        if (!f.exists()) {
            f.mkdirs();
        }
        f = new File(pastaBase + "/" + nomeEntidade);
        if (!f.exists()) {
            f.mkdirs();
        }
        this.nomeEntidade = nomeEntidade;
        this.construtor = construtor;
        arquivo = new RandomAccessFile(pastaBase + "/" + nomeEntidade + "/arquivo.db", "rw");
        indiceID = new HashExtensivel<>(ParIDEndereco.class.getConstructor(), 
            4,
            pastaBase + "/" + nomeEntidade + "/indiceID.diretorio.db",
            pastaBase + "/" + nomeEntidade + "/indiceID.cestos.db");
